...
```{r, include = FALSE}
## Read in species list:
species_list_orthofinder <- read.table(file = "data/OrthoFinder/SpeciesIDs.txt",
                           header = FALSE, sep = "\t")

species_list_ordered <- species_list_orthofinder[order(species_list_orthofinder$V1), ]

## BBS1
bbs1_data_human_OF <- read.table(file = "data/OrthoFinder/5_29768_match.txt",
                   header = FALSE)
...
bbs1_data_insect_OF <- read.table(file = "data/OrthoFinder/5_29768_rep_match.txt",
                   header = FALSE)
...
## BBS2
bbs2_data_human_OF <- read.table(file = "data/OrthoFinder/5_30321_match.txt",
                   header = FALSE)
...
bbs2_data_insect_OF <- read.table(file = "data/OrthoFinder/5_30321_rep_match.txt",
                   header = FALSE)
...
## BBS4
bbs4_data_human_OF <- read.table(file = "data/OrthoFinder/5_30864_match.txt",
                   header = FALSE)
...
bbs4_data_insect_OF <- read.table(file = "data/OrthoFinder/5_30864_rep_match.txt",
                   header = FALSE)
...
## BBS5
bbs5_data_human_OF <- read.table(file = "data/OrthoFinder/5_32419_match.txt",
                   header = FALSE)
...
bbs5_data_insect_OF <- read.table(file = "data/OrthoFinder/5_32419_rep_match.txt",
                   header = FALSE)
...
## BBS7
bbs7_data_human_OF <- read.table(file = "data/OrthoFinder/5_33401_match.txt",
                   header = FALSE)
...
bbs7_data_insect_OF <- read.table(file = "data/OrthoFinder/5_33401_rep_match.txt",
                   header = FALSE)
...
(seq_sim_plot_BLAST <- seq_sim_plot(data_sub_BLAST, 21))
#(seq_sim_plot_OF <- seq_sim_plot(data_sub_OF, 21))
...
bbs_boxplot_BLAST <- ggplot(data = subset(data_sub_BLAST,
       ...
       aes(x = locus,
           y = pident.x)) +
  xlab("Locus") +
  ylab(expression(paste(italic("H. sapiens"), " vs Insect - Percentage ID [%]"))) +
  geom_boxplot() +
  theme_bw() +
  theme(axis.title = element_text(face = "bold",
                                  size = 12)) +
...
(combined_plot <- ggarrange(seq_sim_plot_BLAST,
          bbs_boxplot_BLAST,
          ncol = 1,
          nrow = 2,
          heights = c(2,1),
